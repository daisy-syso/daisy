    ...
    get do
      ...
      information_type.each do |it|
        ...
        if it.parent_id.blank?
          ...
          select_infos = "(select * from informations where id in (#{ids.join(',')}) and is_top is not true order by str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc)"
        else
          ...
          select_infos = part_infors + " union " + "(select * from informations where id in (#{ids.join(',')}) and is_top is not true order by str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc)"
        end
