  ...
  namespace :health_infors do
    get do
      ...
      information_type.each do |it|
        ...
        top_infors = "(select * from informations where information_type_id = #{parent_id} and is_top is true and types = 0 order by str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc)"
        if it.parent_id.blank?
          ids = it.children_items.map(&:id) + [it.id]
          select_infos = "(select * from informations where information_type_id in (#{ids.join(',')}) and is_top is not true and types = 0 order by str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc)"
        else
          part_infors = "(select * from informations where information_type_id = #{it.id} and is_top is not true and types = 0 order by str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc)"
          ids = (it.parent_item.children_items.map(&:id) + [it.parent_item.id]).delete_if{|i| i == it.id}
          select_infos = part_infors + " union " + "(select * from informations where information_type_id in (#{ids.join(',')}) and is_top is not true and types = 0 order by str_to_date(created_at,'%Y-%m-%d %H:%i:%s') desc)"
        end
