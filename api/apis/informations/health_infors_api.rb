  ...
  namespace :health_infors do
    get do
      ...
      information_type.each do |it|
        ...
        top_infors = "(select *, 1 as rank from informations where information_type_id = #{parent_id} and is_top is true and types = 0)"
        ...
        info_sql = "select * from ( " + top_infors + " union " + select_infos + " ) as infors order by rank asc, str_to_date(infors.created_at,'%Y-%m-%d %H:%i:%s') desc limit 8 offset #{((params[:page] || 1).to_i - 1) * 8}"
