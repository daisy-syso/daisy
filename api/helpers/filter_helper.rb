...
# 注射美容
      ...
      h4 = {
        ...
        "01" => %w(不限 阴道分泌物检查 细菌性阴道病监测（BV）),
        # 尿液检验
        "01" => %w(不限 尿妊娠试验 尿HL检测 尿十一项+尿沉渣 尿微量白蛋白),
        # 通用检查
        "01" => %w(不限 单纯疱疹病毒抗体测定四项 淋球菌培养 衣原体抗原 支原体培养及药敏 宫颈刮片细胞学检查 标本病例检查 弓形体抗原测定 梅毒快速血浆反应实验（TRUST） 梅毒螺旋体异性抗体（TPPA） TCT脱落细胞检查),
        # 阴道检查
        "01" => %w(不限 电子数码阴道镜检查 妇科常规检查),
        # B超
        "01" => %w(不限 阴道彩超 阴道黑白超 腹部彩色超声检查 子宫附件B超 孕检彩色超声检查),
        # 血液
        "01" => %w(不限 全血细胞分析 ABO血型定型 Rb(D)定型 凝血四项（PT,TT,APTT,FIB） 乙肝表面抗原 术前四项（乙肝两对半、RPR、抗HCV、抗HIV） 不孕五项（化学发光法）优生五项（化学发光法） 性激素六项 血清泌乳素测定 血清促黄体生成素测定 血清促卵泡刺激素测定 血清绒毛促性激素定量（血-HCG） 甲功三项 甲功五项 风湿四项 孕酮测定 免疫球蛋白四项 唐氏综合症Ⅱ期3联 凝血二项)
      }
      ...
      h5 = {
        ...
        # 灸法
        "00" => %w(不限 灸法(艾柱灸) 灸法(艾条灸) 灸法(艾箱灸) 灸法(天灸) 灸法(其他灸) 隔物灸法(隔姜灸) 隔物灸法(药饼灸) 隔物灸法(隔盐灸) 隔物灸法(其它灸) 灯火灸 药线点灸 拔罐疗法(火罐)（3罐/次） 拔罐疗法(电火罐)（3罐/次） 拔罐疗法(闪罐)（3罐/次） 拔罐疗法(着罐)（3罐/次） 拔罐疗法(电罐)（3罐/次） 拔罐疗法(磁疗罐)（3罐/次） 拔罐疗法(真空拔罐)（3罐/次） 拔罐疗法(其它罐)（3罐/次） 药物罐 水罐 督灸 大灸 雷火灸 太乙神针灸),
        # 推拿疗法
        ...
        # 针刺
        "00" => %w(不限 普通针刺（体针）（5穴位/次） 普通针刺(快速针)（5穴位/次） 普通针刺(磁针)（5穴位/次） 普通针刺(金针)（5穴位/次） 普通针刺(姜针)（5穴位/次） 普通针刺(药针)（5穴位/次） 其它普通针刺（5穴位/次） 温针（5穴位/次） 手指点穴（5穴位/次） 馋针 微针针刺 锋钩针 头皮针 眼针（单眼） 梅花针 七星针 火针(3穴位) 电火针(3穴位) 埋针治疗 耳针(单耳) 芒针 电针(普通电针)（2穴位）浮针 微波针（2穴位） 激光针（2穴位） 磁热疗法（2穴位） 放血疗法 穴位注射 穴位封闭 自血疗法 穴位贴敷治疗 内脏平滑肌痉挛性疼痛止痛敷贴 杵针 圆针),
        # 中医肛肠
        "00" => %w(不限 直肠脱出复位治疗 三度直肠脱垂复位治疗 直肠周围硬化剂注射治疗 内痔硬化剂注射治疗(枯痔治疗) 高位复杂肛瘘挂线治疗 血栓性外痔切除术 环状混合痔切除术 环状混合痔脱出嵌顿切除术 混合痔外剥内扎术 肛外括约肌折叠术 直肠前突修补术 肛瘘封堵术 肛周药物注射封闭术 肛周坏死性筋膜炎清创术 肛门直肠周围脓腔搔刮术 直肠前突出注射术 直肠脱垂注射术) ,
        # 中医骨伤
        "00" => %w(不限 骨折橇拨复位术 骨折经皮钳夹复位术 骨折闭合复位经皮穿刺（钉）内固定术 骨折闭合复位经皮穿刺（钉）内固定术四肢长骨干、近关节加收 关节脱位手法整复术 陈旧性脱位关节脱位手法整复术 髋关节脱位手法整复术 下颌关节脱位手法整复术 指(趾)间关节脱位手法整复术 骨折外固定架固定术 骨折外固定架固定术复查调整 骨折夹板外固定 骨折夹板外固定复查调整 关节错缝术 麻醉下腰椎间盘突出症大手法治疗 关节粘连传统松解术 中医定向透药疗法 腱鞘囊肿挤压术 腱鞘囊肿挤压术 腰间盘三维牵引复位术),
        # 中医特殊疗法
        "00" => %w(不限 白内障针拨术（单眼） 白内障针拨术（双眼） 白内障针拨吸出术（单眼） 白内障针拨吸出术（双眼） 白内障针拨吸出术（双眼） 白内障针拨套出术（双眼） 眼结膜囊穴位注射（单眼） 眼结膜囊穴位注射（双眼） 小针刀治疗 钩针疗法 刃针治疗 红皮病清消术 扁桃体烙法治疗 鼻中隔烙法治疗 药线引流治疗 耳咽中药吹粉治疗 中药硬膏热贴敷治疗 中药直肠滴入治疗 刮痧治疗 烫熨治疗 体表瘘管切开搔爬术 足底反射治疗),
        # 中医外治
        "00" => %w(不限 中药化腐清创术 中药涂擦治疗（>10%体表面积） 中药热奄包治疗 中药封包治疗 中药熏洗治疗 中药熏洗治疗（半身） 中药熏洗治疗（全身） 中药蒸汽浴治疗(30分钟) 中药蒸汽浴治疗（>30分钟） 中药塌渍治疗（10%体表面积） 中药塌渍治疗（>10%体表面积） 中药熏药治疗 赘生物中药腐蚀治疗（每赘生物） 挑治 割治)
      }
      case type
      when "andrology"
        h = h1
