module FilterHelper
  ...
  module ClassMethods
    ...
    def form_radio_array_filters_new type, title
      ...
      h1 = { "16" => %w(不限 B超引导下前列腺活检术 前列腺针吸细胞学活检术
...
# 注射美容
      ...
      h4 = {
        # 分泌物检验
        "0" => %w(不限 阴道分泌物检查 细菌性阴道病监测（BV）),
        # 尿液检验
        "0" => %w(不限 尿妊娠试验 尿HL检测 尿十一项+尿沉渣 尿微量白蛋白),
        # 通用检查
        "0" => %w(不限 单纯疱疹病毒抗体测定四项 淋球菌培养 衣原体抗原 支原体培养及药敏 宫颈刮片细胞学检查 标本病例检查 弓形体抗原测定 梅毒快速血浆反应实验（TRUST） 梅毒螺旋体异性抗体（TPPA） TCT脱落细胞检查),
        # 阴道检查
        "0" => %w(不限 电子数码阴道镜检查 妇科常规检查),
        # B超
        "0" => %w(不限 阴道彩超 阴道黑白超 腹部彩色超声检查 子宫附件B超 孕检彩色超声检查),
        # 血液
        "0" => %w(不限 全血细胞分析 ABO血型定型 Rb(D)定型 凝血四项（PT,TT,APTT,FIB） 乙肝表面抗原 术前四项（乙肝两对半、RPR、抗HCV、抗HIV） 不孕五项（化学发光法）优生五项（化学发光法） 性激素六项 血清泌乳素测定 血清促黄体生成素测定 血清促卵泡刺激素测定 血清绒毛促性激素定量（血-HCG） 甲功三项 甲功五项 风湿四项 孕酮测定 免疫球蛋白四项 唐氏综合症Ⅱ期3联 凝血二项)
      }
      case type
      when "andrology"
        h = h1
